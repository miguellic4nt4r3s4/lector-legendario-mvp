...
import { Backpack, Check, ChevronRight, Compass, Lock, Medal, ScrollText, Shield, Sparkles, Swords } from "lucide-react";
  ...
  return (
    <section className="adventure-stage relative isolate min-h-[960px] overflow-hidden sm:min-h-[920px] lg:min-h-[calc(100svh-65px)] lg:max-h-[900px]" aria-labelledby="aventura-mundo-actual">
      ...
      <div className="absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 px-4 pt-5 sm:px-7 sm:pt-7 lg:px-10 lg:pt-8">
        <div className="min-w-0 adventure-world-title">
          ...
          <h1 id="aventura-mundo-actual" className="mt-1 font-adventure text-3xl text-primary sm:text-5xl lg:text-6xl">Bosque de las Palabras</h1>
        </div>
        <div className="hud-level shrink-0" aria-label={`Nivel ${nivel}`}><span>Nivel</span><strong>{String(nivel).padStart(2, "0")}</strong></div>
      </div>

      <aside id="mi-heroe" className="player-hud absolute left-4 top-[525px] z-30 w-[calc(100%-2rem)] sm:left-7 sm:top-[580px] sm:w-[360px] lg:left-10 lg:top-[31%] lg:w-[300px]" aria-label={`Datos de ${nombre}`}>
      ...
      <aside className="absolute inset-x-4 top-[690px] z-30 sm:inset-x-auto sm:bottom-7 sm:right-7 sm:top-auto sm:w-[430px] lg:bottom-[72px] lg:right-10 lg:top-auto lg:w-[390px]">
        <section className="quest-focus reveal-up" aria-labelledby="mision-actual">
          <div className="flex items-center gap-2 text-accent"><ScrollText className="h-4 w-4" aria-hidden /><p className="text-[10px] font-bold uppercase">Punto de misión</p></div>
      ...
      <div className="absolute bottom-[118px] left-10 z-30 hidden lg:block">
        <div className="flex items-center justify-between gap-4"><p className="flex items-center gap-2 text-[10px] font-bold uppercase text-accent"><Swords className="h-4 w-4" aria-hidden /> Equipo</p><Button asChild variant="ghost" size="icon" className="h-7 w-7"><Link to="/personalizar" search={{ categoria: "top" }} aria-label="Abrir mochila"><Backpack aria-hidden /></Link></Button></div>
      ...
      <div id="insignias" className="absolute bottom-8 left-10 z-30 hidden lg:block" aria-label="Trofeos">
      ...
      <div className="adventure-path-marker absolute bottom-5 left-1/2 z-20 hidden -translate-x-1/2 items-center gap-2 text-xs font-semibold text-primary lg:flex"><Compass className="h-4 w-4" aria-hidden /> Expedición activa</div>
    </section>
  );
}