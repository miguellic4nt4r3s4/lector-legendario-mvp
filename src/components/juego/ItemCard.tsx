...
export function ItemCard({ item, desbloqueada, equipada, cargando, onEquipar }: Props) {
  return <article className={cn("group relative overflow-hidden rounded-lg border bg-card/80 p-4 transition-transform duration-200 hover:-translate-y-0.5", estiloMarcoRareza(item.rarity), !desbloqueada && "opacity-65 grayscale-[35%]")}>
    <div className="relative aspect-square overflow-hidden rounded-md border border-border bg-background/55">
      ...
      <AvatarModular configuracion={{ [item.category]: item.asset }} className="absolute inset-x-[19%] bottom-0 border-0 bg-transparent" nombre={`Vista de ${item.name}`} />
    ...
    <p className={cn("mt-3 min-h-8 text-xs font-semibold uppercase", equipada ? "text-primary" : desbloqueada ? "text-success" : "text-muted-foreground")}>{equipada ? "Equipado" : desbloqueada ? "Desbloqueado" : `Bloqueado · ${item.unlock_condition}`}</p>
